...
def szam_penz(x, nev):
    try:
        return int(float(str(x).replace(" ", "").replace(",", ".").replace("Ft", "")))
    except:
...
class KatalogusTab:
    def __init__(self, tab):
        self.tab = tab
        self.katalogus = Katalogus()
        self.kivalasztott = None

        fo = ttk.Frame(tab, padding=10)
    ...
    def sor(self, a):
        return f"{a.rendszam} | {a.marka} {a.modell} | {a.ev} | {a.szin} | {penz_formaz(a.ar)}"
    ...
    def kilepes(self):
        self.tab.winfo_toplevel().destroy()

def main():
    gyoker = tk.Tk()
    gyoker.title("Autókatalógus – 20 autóval, mentéssel és kilépéssel")
    jegyzet = ttk.Notebook(gyoker)
    jegyzet.pack(fill="both", expand=True)
    f1 = ttk.Frame(jegyzet)
    jegyzet.add(f1, text="Autókatalógus")
    KatalogusTab(f1)
    gyoker.mainloop()
